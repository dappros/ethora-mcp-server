...
export const HOSTED_INSTRUCTIONS = [
  ...
  "Typical flow after auth: `ethora-app-create` to create an app, then `ethora-app-select` to make it current, then chats/users/agents tools (`ethora-chat-create`, `ethora-agent-create`, `ethora-agent-invite`, `ethora-message-send` with waitForReplySec, ...). For a website chat widget: `ethora-agent-activate { agentId, chatJid }` then `ethora-widget-snippet-get`.",
  ...
  "App credentials are never included in tool results: appSecret, tenantSecret, appToken and passwords come back as [redacted]. When a snippet or config needs the appToken, call `ethora-app-credentials-reveal { appId, confirm: true }`; the App Secret is only available in the web dashboard API tab.",
...
export const STDIO_INSTRUCTIONS = [
  ...
  "Configuration comes from env vars: ETHORA_API_URL (API base, defaults to Ethora Cloud), ETHORA_APP_JWT (app JWT needed by login/register), optional ETHORA_B2B_TOKEN for server-to-server automation. `ethora-session-configure` can set them at runtime.",
  "Auth: call `ethora-status`, then `ethora-user-login` (email + password) or `ethora-user-register`; or switch to app/B2B auth with `ethora-auth-mode-set` / `ethora-auth-mode-set` when you hold those tokens.",
  ...
  "App credentials (appSecret, tenantSecret, appToken, passwords) are redacted from tool results; use `ethora-app-credentials-reveal { appId, confirm: true }` for the appToken.",
...
const AUTHENTICATED_COMMON = [
  ...
  "Typical flow: `ethora-app-list` / `ethora-app-select` to pick an app, or `ethora-app-create` for a new one, then chats, users and agents tools (`ethora-agent-create`, `ethora-agent-invite`, `ethora-message-send`, ...).",
  "Website chat widget: `ethora-agent-activate { agentId, chatJid }` after inviting the agent, then `ethora-widget-snippet-get` for the <script> tag (`ethora-help { goal: \"widget\" }` lists the steps).",
